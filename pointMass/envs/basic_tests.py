...
p.connect(p.GUI)
# monastryId = concaveEnv =p.createCollisionShape(p.GEOM_MESH,fileName=os.path.join(urdfRoot,"samurai_monastry.obj"),flags=p.GEOM_FORCE_CONCAVE_TRIMESH)
# orn = p.getQuaternionFromEuler([1.5707963,0,0])
# p.createMultiBody (0,monastryId, baseOrientation=orn)
p.loadSDF(os.path.join(urdfRoot,"stadium.sdf"),useMaximalCoordinates=useMaximalCoordinates)
